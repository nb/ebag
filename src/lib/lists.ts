...
export async function getLists(
  ...
  const result = await requestEbag<ListSummary[]>(
    config,
    session,
    "/lists/json",
  );
  return result.data.map((list) => ({
    ...
    isReadOnly:
      (list as { is_read_only?: boolean }).is_read_only ?? list.isReadOnly,
    products: (
      list as { products?: { product_id: number; quantity: number }[] }
    ).products?.map((p) => ({
...
export async function addToList(
  ...
  const result = await requestEbag(
    config,
    session,
    `/lists/${listId}/items/update`,
    {
      method: "POST",
      headers: {
