...
const OUTPUT_DIR =
  process.env.EBAG_CAPTURE_DIR || path.join(process.cwd(), "captures");
...
async function main() {
  ...
  if (
    ...
    console.log(
      "Auto-login not configured. Please log in manually in the opened browser.",
    );
  ...
  console.log(
    "Now perform the actions in the browser to capture network calls.",
  );
  ...
  fs.writeFileSync(
    outputPath,
    JSON.stringify({ markers, captures }, null, 2),
    "utf8",
  );
